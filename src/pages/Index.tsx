...
const faqData = [
  ...
  { id: "cm-2", category: "Changes & Modifications", question: "What counts as proof of delivery?",    answer: "Any agreed-upon evidence: photos, videos, documents, or the hustler marking the gig done from their side. Final release always requires the client PIN." },
...
export default function Index() {
  ...
  const filteredFAQ = useMemo(() => {
    ...
    return faqData.filter(i => i.question.toLowerCase().includes(q) || i.answer.toLowerCase().includes(q));
  }, [searchQuery]);
    ...
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <main>
        ...
        <section className="relative min-h-[92vh] flex flex-col items-center justify-center text-center px-4 pt-24 pb-16 overflow-hidden">
          <div className="pointer-events-none absolute inset-0 opacity-[0.035] dark:opacity-[0.06]" style={{ backgroundImage: "linear-gradient(to right, currentColor 1px, transparent 1px), linear-gradient(to bottom, currentColor 1px, transparent 1px)", backgroundSize: "48px 48px" }} />
          ...
          <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/5 px-4 py-1.5 text-xs font-medium text-primary">
            <Shield className="h-3 w-3" /> Escrow-backed · KYC-verified · South Africa
          </div>
          <h1 className="relative text-5xl md:text-7xl font-extrabold tracking-tight leading-[1.05] mb-6 max-w-3xl">
            The Service Marketplace<br /><span className="text-primary">Built on Trust</span>
          </h1>
          <p className="relative text-lg text-muted-foreground mb-10 max-w-lg leading-relaxed">
            Post tasks, hire verified hustlers, and pay with confidence. Funds are held in escrow and only released when the job is done.
          </p>
          ...
          <div className="relative flex flex-col sm:flex-row items-center gap-8 sm:gap-16 text-center">
            {[{ value: "100%", label: "Escrow Protected" }, { value: "KYC", label: "Verified Users Only" }, { value: "PIN", label: "Secure Fund Release" }].map(({ value, label }) => (
              <div key={label}>
        ...
        <section id="features" className="scroll-mt-24 container pb-24">
          ...
          <div className="grid md:grid-cols-3 gap-5 max-w-5xl mx-auto">
            {[
              { icon: Lock,   title: "Escrow Protection", desc: "Client funds are locked on gig acceptance. The hustler only gets paid when you release the PIN — no disputes, no chargebacks.",                accent: "text-primary",     bg: "bg-primary/5 border-primary/15"     },
              { icon: Zap,    title: "Instant Matching",  desc: "Post a gig and get matched with hustlers in your area. Our open marketplace surfaces the right talent fast.",                                    accent: "text-amber-500",   bg: "bg-amber-500/5 border-amber-500/15" },
              { icon: Shield, title: "KYC Verification",  desc: "Every user is identity-verified before transacting. You always know exactly who you're dealing with.",                                           accent: "text-emerald-500", bg: "bg-emerald-500/5 border-emerald-500/15" },
        ...
        <section id="how-it-works" className="scroll-mt-24 py-24 border-y border-border bg-muted/20">
          <div className="container max-w-5xl mx-auto">
            <div className="text-center mb-16">
              ...
              <p className="text-muted-foreground max-w-md mx-auto">From posting a task to releasing funds — every step is transparent and protected.</p>
            </div>
        ...
        <section id="faq" className="scroll-mt-24 container py-24">
          <div className="max-w-3xl mx-auto">
            <div className="text-center mb-12">
              ...
              <p className="text-muted-foreground max-w-md mx-auto mb-8">Everything you need to know about GigHold.</p>
              <div className="relative max-w-lg mx-auto">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <input type="text" placeholder="Search questions…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-11 pr-4 py-2.5 bg-card border border-border rounded-xl text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all" />
              </div>
            </div>
            <div className="space-y-4">
                    ...
                    <Accordion type="single" collapsible className="w-full">
                      {section.items.map(item => (
                        <AccordionItem key={item.id} value={item.id} className="border-border">
                          <AccordionTrigger className="text-left text-sm hover:no-underline hover:text-primary transition-colors py-4 font-medium">{item.question}</AccordionTrigger>
              ...
              {filteredFAQ.length === 0 && (
                <div className="rounded-2xl border border-border bg-card p-10 text-center">
                  <p className="text-muted-foreground text-sm">No results for <span className="text-primary font-medium">"{searchQuery}"</span></p>
                </div>
              )}
            </div>
          </div>
      ...
      <footer className="border-t border-border bg-card/60">
        <div className="container mx-auto px-6 py-14">
          <div className="grid md:grid-cols-4 gap-10 mb-10">
            ...
            {[
              { heading: "Product", links: [{ label: "Features", action: () => scrollTo("features") }, { label: "How it Works", action: () => scrollTo("how-it-works") }, { label: "FAQ", action: () => scrollTo("faq") }, { label: "Pricing", href: "/pricing" }] },
              { heading: "Company", links: [{ label: "Contact Us", href: "/contact" }] },
              { heading: "Legal",   links: [{ label: "Terms of Service", href: "/terms" }, { label: "Privacy Policy", href: "/privacy" }] },
            ].map(col => (
              <div key={col.heading}>
